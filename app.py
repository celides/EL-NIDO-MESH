...
def llamar_groq(mensajes, system_prompt):
    ...
    if r.status_code == 429:
        import time as _time; _time.sleep(12); r = requests.post(
            "https://api.groq.com/openai/v1/chat/completions",
            headers={"Authorization": f"Bearer {GROQ_API_KEY}", "Content-Type":"application/json"},
            json={"model":"llama-3.1-8b-instant",
                  "messages":[{"role":"system","content":system_prompt}] + mensajes,
...
with col_ctrl:
    ...
    for agente, cfg in AGENTES.items():
        ...
        if not encendido:
            ...
            ind_color = "#00ff88"
            ind_emoji = "🟢"
            glow      = "0 0 10px #00ff8888"
        elif nivel == "error":
            ind_color = "#ff4444"
            ind_emoji = "🔴"
            glow      = "0 0 10px #ff444488"
        elif nivel == "warn":
            ind_color = "#ffaa00"
            ind_emoji = "🟡"
            glow      = "0 0 10px #ffaa0088"
        else:
            ind_color = cfg["color"]
            ind_emoji = cfg["emoji"]
            glow      = f"0 0 8px {cfg['color']}55"

        borde_dest = f"2px solid {cfg['color']}" if es_dest else "1px solid #1a1a35"

        # Panel HTML del agente con botón toggle integrado
        st.markdown(f"""
<div style="
    background:rgba(10,10,28,0.9);
    border-radius:10px;
...
">
    <div style="display:flex;align-items:center;justify-content:space-between;">
        <div style="display:flex;align-items:center;gap:8px;">
            <span style="font-size:1.1em;">{cfg['emoji']}</span>
            <div>
                <div style="font-family:Orbitron,monospace;font-size:0.72em;
                            color:{cfg['color']};letter-spacing:0.1em;font-weight:700;">
                    {"▶ " if es_dest else ""}{agente}
                </div>
                <div style="font-family:Rajdhani,sans-serif;font-size:0.7em;color:#445566;">
                    {cfg['rol']}
                </div>
            </div>
        </div>
        ...
        # Dos botones lado a lado: Seleccionar | Encender/Apagar
        b1, b2 = st.columns(2)
        with b1:
            lbl_sel = "▶ SELECCIONADO" if es_dest else f"SELECCIONAR"
            if st.button(lbl_sel, key=f"sel_{agente}", use_container_width=True):
                st.session_state.destinatario = agente
                st.rerun()
        with b2:
            lbl_tog = "⏸ APAGAR" if encendido else "▶ ENCENDER"
            if st.button(lbl_tog, key=f"tog_{agente}", use_container_width=True):
                st.session_state.agente_encendido[agente] = not encendido
                st.rerun()

        # Diagnóstico debajo
        msg_diag = diag.get("mensaje", "Sin actividad aún.")
        ...
        if accion:
            diag_html += f'<br><span style="opacity:0.75;font-size:0.9em;">{accion}</span>'
        if url_rec and nivel == "error":
            diag_html += f'<br><a href="{url_rec}" target="_blank" style="color:#88aaff;font-size:0.85em;">→ {url_rec.split("/")[2]}</a>'
        diag_html += "</div>"
